...
'''In the testing phase, we use these optimal hyper-parameter to save attention matrix, make sure they match the checkpoint'''
...
def test_model(file_path, model_path, output_file, metric_file, batch_size=128, GPU_num=0, align=True):
    device = torch.device(f'cuda:{GPU_num}' if torch.cuda.is_available() else 'cpu')
    if torch.cuda.is_available():print(f'Testing model on GPU{GPU_num}.') 
    df = pd.read_csv(file_path)
    x, y = df[['TCR', 'epitope']], df['Label']
    x, y = dropInvalid(x, y)
    dataset_test = TEPDataset(x, y, align=align)
    ...
    write_metrics(metric_file, "-----------------{}--------------\n".format(model_path))
    write_metrics(metric_file, "ACC,{:.3f}\n".format(ACC))
    write_metrics(metric_file, "AUC,{:.3f}\n".format(AUC))
    write_metrics(metric_file, "AUPR,{:.3f}\n".format(AUPR))
    write_metrics(metric_file, "Recall,{:.3f}\n".format(Recall))
    write_metrics(metric_file, "Precision,{:.3f}\n".format(Precision))
    write_metrics(metric_file, "F1,{:.3f}\n".format(F1))
    

def write_metrics(file_path,result):
    f = open(file_path, "a", encoding="gbk", newline="")
    f.write(result)
    f.close()       
    
def create_parser():
    parser = argparse.ArgumentParser(description="Test TEPCAM using test dataset",formatter_class=argparse.ArgumentDefaultsHelpFormatter,)
    ...
    parser.add_argument("--output_file",dest="output_file",type=str, help="File for saving the output result", default='./output.csv', required=False)
    parser.add_argument("--batch_size", dest="batch_size", type=int, help="number of samples per-batch", default=128, required=False)
    parser.add_argument("--metric_file", dest="metric_file", type=str, default='./metrics.txt', required=False)
...
if __name__=="__main__":
    ...
    print(f"Model test finish! Save output score in {args.output_file} and metrics in {args.metric_file}")
